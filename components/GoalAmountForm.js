...
function GoalAmountForm({ goalAmount, setGoalAmount }) {
  const [editMode, setEditMode] = useState(false); // 編集モードの状態を追跡
  const [inputValue, setInputValue] = useState(goalAmount); // 入力フィールドの状態を追跡

  const handleSave = () => {
    setGoalAmount(inputValue); // 親コンポーネントのgoalAmountを更新
  ...
  const toggleEdit = () => {
    ...
    setInputValue(goalAmount); // 現在の目標金額を入力フィールドにセット
  };
      ...
      {editMode ? (
        <View style={styles.inputContainer}>
          <TextInput
            ...
            onChangeText={(text) => setInputValue(text.replace(/[^0-9]/g, ""))} // 数字以外の入力を除去
          />
          <Button title="保存" onPress={handleSave} />
        </View>
      ) : (
        <View style={styles.displayContainer}>
          <Text style={styles.displayText}>目標金額: {goalAmount}円</Text>
          <Button title="編集" onPress={toggleEdit} />
        </View>
      )}
    </View>
  );
